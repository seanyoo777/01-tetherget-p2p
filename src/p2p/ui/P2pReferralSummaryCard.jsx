...
export function P2pReferralSummaryCard({ theme, summary = MOCK_REFERRAL_SUMMARY, formatNumber }) {
  ...
  return (
    <div className={`rounded-2xl border p-4 sm:p-5 ${p2pSurfaceCard(isDark)}`}>
      <div className="flex flex-wrap items-end justify-between gap-2">
        <div>
          <div className="text-[10px] font-black uppercase tracking-wider text-emerald-500">Referral (mock)</div>
          <div className="mt-1 text-lg font-black">레퍼럴 요약</div>
...
function Stat({ label, value, theme, accent }) {
  return (
    <div className={`rounded-xl border p-2.5 ${theme.cardSoft} ${accent ? "border-emerald-500/30" : ""}`}>
